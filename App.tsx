    ...
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 border border-red-100 p-6 text-center">
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 px-4 py-2 bg-gray-100 text-gray-700 font-bold rounded-xl">
            Cancelar
          </button>
          <button
            onClick={() => {
            ...
            className="flex-1 px-4 py-2 bg-red-600 text-white font-bold rounded-xl shadow-lg shadow-red-100"
          >
            Excluir
          </button>
        </div>
...
const App: React.FC = () => {
  ...
  const toggleWithdrawal = async (studentId: string, partId: string) => {
    ...
    const updated = existing
      ? withdrawals.filter(w => !(w.studentId === studentId && w.partId === partId))
  ...
  const stockSummary = useMemo(() => {
    return [...parts]
      .sort((a, b) => sortTaskIds(a.id, b.id))
      .map(part => {
        const entries = transactions
          .filter(t => t.partId === part.id && t.type === TransactionType.ENTRY)
          .reduce((sum, t) => sum + t.quantity, 0);

        const exits = transactions
          .filter(t => t.partId === part.id && t.type === TransactionType.EXIT)
          .reduce((sum, t) => sum + t.quantity, 0);
      ...
      <div className="min-h-screen bg-[#F8FAFC] flex flex-col md:flex-row">
        ...
        <aside className="hidden md:flex flex-col w-64 bg-gray-900 h-screen sticky top-0 shadow-2xl z-50">
          <div className="p-8 border-b border-gray-800 bg-gray-950 flex justify-between items-start">
            <div>
              ...
              <h1 className="text-white font-black text-2xl tracking-tighter italic leading-none">
                SENAI
                <br />
                <span className="text-gray-400 font-medium text-[10px] tracking-widest uppercase not-italic">
                  Usinagem
                </span>
              </h1>
            </div>
        ...
        <main className="flex-1 p-4 md:p-10 pb-24 min-w-0">
          <Routes>
            ...
            <Route
              ...
              element={
                <MaterialWithdrawals
                  ...
                  students={students}
                  parts={[...parts].sort((a, b) => sortTaskIds(a.id, b.id))}
                />
              }
            />
            <Route
            ...
            <Route
              path="/students"
              element={<StudentsManager students={students} onSave={handleSaveStudent} onDelete={handleDeleteStudent} />}
            />
          </Routes>
        </main>
  ...
  return (
    <Link
      ...
      className={`flex items-center space-x-3 p-3 rounded-xl transition-all ${
        active ? 'bg-red-600 text-white shadow-lg' : 'text-gray-400 hover:bg-gray-800 hover:text-white'
      }`}
    >
      {icon}
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <header className="bg-white p-8 rounded-[2.5rem] shadow-xl shadow-gray-200/50 border border-gray-100 flex flex-col md:flex-row items-center gap-10">
        ...
        <div className="text-center md:text-left flex-1 space-y-3">
          <div className="space-y-1">
            ...
            <p className="text-lg md:text-xl font-bold text-gray-500 uppercase tracking-tight">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-3 pt-2">
            ...
            <span className="inline-flex items-center gap-1.5 px-4 py-1.5 bg-blue-50 text-blue-600 text-xs font-black uppercase tracking-widest rounded-full border border-blue-100">
              <Users size={14} /> Controle de Alunos
            </span>
          </div>
        </div>
      </header>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-7 rounded-3xl shadow-sm border border-gray-100 flex items-center space-x-5 transition-all hover:shadow-md">
          ...
          <div>
            <p className="text-sm font-bold text-gray-400 uppercase tracking-wider">Alunos</p>
        ...
        <div className="bg-white p-7 rounded-3xl shadow-sm border border-gray-100 flex items-center space-x-5 transition-all hover:shadow-md">
          ...
          <div>
            <p className="text-sm font-bold text-gray-400 uppercase tracking-wider">Estoque</p>
            <p className="text-3xl font-black text-gray-900">{summary.reduce((acc, curr) => acc + curr.balance, 0)}</p>
          </div>
        </div>
        <div className="bg-white p-7 rounded-3xl shadow-sm border border-gray-100 flex items-center space-x-5 transition-all hover:shadow-md">
          ...
          <div>
            <p className="text-sm font-bold text-gray-400 uppercase tracking-wider">Entregues</p>
            <p className="text-3xl font-black text-gray-900">{totalEntregas}</p>
          </div>
        </div>
        <div className="bg-white p-7 rounded-3xl shadow-sm border border-gray-100 flex items-center space-x-5 transition-all hover:shadow-md">
          ...
          <div>
            <p className="text-sm font-bold text-gray-400 uppercase tracking-wider">Críticos</p>
            <p className="text-3xl font-black text-gray-900">{itemsCriticos}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-xl font-black text-gray-800 flex items-center gap-2 uppercase italic tracking-tighter">
              <TrendingDown size={22} className="text-orange-500" /> Alertas de Reposição
            </h3>
            <Link
              to="/planning"
          ...
          <div className="space-y-4">
            {summary
              ...
              .map(item => (
                <div
                  key={item.partId}
                  className="flex justify-between items-center p-4 bg-gray-50 hover:bg-red-50/50 rounded-2xl border border-gray-100 transition-colors"
                >
                  <div className="flex items-center gap-3">
                    <div className="h-10 w-10 flex items-center justify-center bg-white rounded-xl font-black text-red-600 border border-red-100">
                      {item.partId}
                    </div>
                    <div>
                      <p className="font-bold text-gray-700 leading-tight">Peça {item.partId}</p>
                      <p className="text-xs text-gray-400 font-medium">Cód: {item.code}</p>
                    </div>
                  </div>
        ...
        <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
          <h3 className="text-xl font-black text-gray-800 mb-6 flex items-center gap-2 uppercase italic tracking-tighter">
            <ArrowLeftRight size={22} className="text-blue-500" /> Ações Rápidas
          </h3>
          <div className="grid grid-cols-2 gap-4">
            <Link
              to="/withdrawals"
              className="group p-6 bg-blue-600 rounded-2xl hover:bg-blue-700 transition-all shadow-lg shadow-blue-100 text-center"
            >
            ...
            <Link
              to="/transactions"
              className="group p-6 bg-gray-900 rounded-2xl hover:bg-black transition-all shadow-lg shadow-gray-200 text-center"
            >
            ...
            <Link
              to="/stock"
              className="group p-6 bg-white border-2 border-gray-100 rounded-2xl hover:border-blue-500 hover:bg-blue-50 transition-all text-center"
            >
            ...
            <Link
              to="/planning"
              className="group p-6 bg-white border-2 border-gray-100 rounded-2xl hover:border-red-500 hover:bg-red-50 transition-all text-center"
            >
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
          <h2 className="text-2xl font-black text-gray-800 uppercase italic tracking-tighter">
            Checklist <span className="text-gray-400 not-italic">de Entrega</span>
          </h2>
        </div>
        <div className="flex flex-wrap gap-2 w-full md:w-auto">
          <div className="relative">
            ...
            <input
              ...
              className="pl-10 pr-4 py-2.5 border-2 border-gray-50 rounded-2xl bg-gray-50 focus:bg-white focus:border-blue-500 transition-all outline-none flex-1"
              value={searchTerm}
              onChange={e => setSearchTerm(e.target.value)}
            />
          </div>
          <select
            className="p-2.5 border-2 border-gray-50 rounded-2xl bg-gray-50 font-bold outline-none focus:border-blue-500 transition-all"
            value={selectedClass}
      ...
      <div className="bg-white rounded-[2rem] shadow-xl shadow-gray-200/50 border overflow-x-auto min-h-[400px]">
        <table className="w-full text-left border-collapse">
          <thead className="bg-gray-900 text-white">
            <tr>
              <th className="p-5 border-r border-gray-800 sticky left-0 bg-gray-900 z-20 uppercase text-xs font-black tracking-widest">
                Estudante
              </th>
              {parts.map(p => (
                <th
                  key={p.id}
                  className="p-3 text-center border-r border-gray-800 text-[10px] font-black min-w-[65px] uppercase tracking-tighter"
                >
                  {p.id}
                </th>
              ))}
          ...
          <tbody>
            {filteredStudents.map((student, idx) => (
              <tr key={student.id} className={idx % 2 === 0 ? 'bg-gray-50/50' : 'bg-white'}>
                <td className="p-4 border-r border-gray-200 font-bold text-sm whitespace-nowrap sticky left-0 bg-inherit z-10 shadow-sm">
                  {student.name}
                </td>
                {parts.map(part => {
                  ...
                  return (
                    <td
                      key={part.id}
                      className={`p-2 border-r border-gray-100 text-center cursor-pointer transition-colors ${
                        active ? 'bg-green-100/50' : 'hover:bg-blue-50'
                      }`}
                      onClick={() => toggleWithdrawal(student.id, part.id)}
                    >
                      <div
                        className={`mx-auto h-7 w-7 rounded-xl border-2 transition-all flex items-center justify-center ${
                          active ? 'bg-green-500 border-green-600 shadow-md shadow-green-100' : 'border-dashed border-gray-300'
                        }`}
                      >
...
const PartsManager: React.FC<{ parts: Part[]; onSave: (p: Part, e: boolean) => void; onDelete: (id: string) => void }> = ({
  parts,
  onSave,
  onDelete
}) => {
  ...
  const handleOpenAdd = () => {
    setEditingPart(null);
    setFormData({ id: '', code: '', name: '', targetQuantity: 0 });
    setIsModalOpen(true);
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-6 rounded-3xl border border-gray-100">
        <h2 className="text-2xl font-black italic uppercase tracking-tighter">
          Tarefas <span className="text-gray-400 not-italic">& Códigos</span>
        </h2>
        <button
          onClick={handleOpenAdd}
          className="bg-red-600 text-white px-6 py-3 rounded-2xl font-black shadow-lg shadow-red-100 flex items-center gap-2 transition-transform hover:scale-105 uppercase tracking-widest text-xs"
        >
          <PlusCircle size={20} /> Nova Peça
        </button>
      </div>
      ...
      <div className="bg-white rounded-[2rem] shadow-sm border overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-gray-50 border-b">
            <tr>
              <th className="p-5 uppercase text-[10px] font-black tracking-widest text-gray-400">Tarefa</th>
              <th className="p-5 uppercase text-[10px] font-black tracking-widest text-gray-400">Código</th>
              <th className="p-5 text-center uppercase text-[10px] font-black tracking-widest text-gray-400">Meta</th>
              <th className="p-5 text-center uppercase text-[10px] font-black tracking-widest text-gray-400">Ações</th>
            </tr>
          </thead>
          <tbody>
            {sortedParts.map(p => (
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingPart ? 'Editar Peça' : 'Nova Peça'}>
        ...
        >
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Identificação</label>
              <input
                type="text"
                placeholder="ex: T1"
                required
                disabled={!!editingPart}
                className="w-full p-4 border-2 border-gray-50 rounded-2xl bg-gray-50 outline-none focus:border-red-500 transition-all font-bold"
                value={formData.id}
                onChange={e => setFormData({ ...formData, id: e.target.value.toUpperCase() })}
              />
            </div>
            <div className="space-y-1">
              <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Meta Turma</label>
              <input
                type="number"
                placeholder="0"
                required
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Código SENAI</label>
            <input
              type="text"
              placeholder="Código SAP/SENAI"
              className="w-full p-4 border-2 border-gray-50 rounded-2xl bg-gray-50 outline-none focus:border-red-500 transition-all font-bold"
              value={formData.code}
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Nome da Peça</label>
            <input
              type="text"
          ...
          <button
            type="submit"
            className="w-full bg-red-600 text-white font-black py-4 rounded-2xl shadow-lg shadow-red-100 uppercase tracking-widest text-sm transition-all hover:bg-red-700"
          >
            Salvar Alterações
          </button>
        </form>
  ...
  <div className="space-y-6">
    <div className="flex justify-between items-center bg-white p-6 rounded-3xl border border-gray-100">
      <h2 className="text-2xl font-black uppercase italic tracking-tighter">
        Estoque <span className="text-gray-400 not-italic">Consolidado</span>
      </h2>
      <button onClick={() => window.print()} className="bg-gray-100 p-3 rounded-2xl hover:bg-gray-200 transition-colors">
        <Download size={22} />
      </button>
    </div>
            ...
            <tr key={item.partId} className="hover:bg-gray-50 transition-colors">
              ...
              <td
                className={`p-5 text-center font-black text-lg ${
                  item.balance < 5 ? 'text-red-600 bg-red-50' : 'text-blue-700 bg-blue-50/30'
                }`}
              >
                {item.balance}
              </td>
  ...
  <div className="space-y-6">
    <div className="bg-gray-900 p-10 rounded-[2.5rem] shadow-2xl text-white relative overflow-hidden">
      ...
      <h2 className="text-4xl font-black uppercase italic tracking-tighter">
        Compra <span className="text-red-600">Necessária</span>
      </h2>
      <p className="text-gray-400 font-medium max-w-md mt-2 tracking-wide uppercase text-xs">
        Planejamento estratégico baseado no déficit para as turmas atuais.
      </p>
    </div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {summary
        ...
        .map(item => (
          <div
            key={item.partId}
            className="bg-white p-8 rounded-[2rem] shadow-sm border-t-8 border-red-600 flex justify-between items-center group transition-all hover:shadow-xl hover:-translate-y-1"
          >
            <div>
            ...
            <div className="text-right">
              <span className="text-[10px] font-black text-red-500 block uppercase tracking-widest mb-1">Déficit</span>
              <span className="text-5xl font-black text-red-600 tabular-nums">{item.toBuy}</span>
            </div>
          </div>
        ))}
...
const StudentsManager: React.FC<{
  ...
  const filtered = useMemo(
    () => students.filter(s => s.class === selectedClass).sort((a, b) => sortAlphabetically(a.name, b.name)),
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-3xl border border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
        <h2 className="text-2xl font-black italic uppercase tracking-tighter">
          Alunos <span className="text-gray-400 not-italic">& Turmas</span>
        </h2>
        <div className="flex gap-3 w-full md:w-auto">
          <select
            className="p-3 border-2 border-gray-50 rounded-2xl bg-gray-50 font-black outline-none focus:border-blue-500 transition-all text-sm"
            value={selectedClass}
            onChange={e => setSelectedClass(e.target.value)}
          >
            {CLASSES.map(c => (
          ...
          <button
            onClick={() => {
              setEditingId(null);
              setFormData({ name: '', class: selectedClass });
              setIsModalOpen(true);
            }}
            className="bg-blue-600 text-white px-6 py-3 rounded-2xl font-black shadow-lg shadow-blue-100 flex items-center gap-2 uppercase tracking-widest text-xs"
          >
                ...
                <td className="p-5 text-center">
                  <div className="flex justify-end gap-3">
                    <button
                      onClick={() => {
                        setEditingId(s.id);
                        setFormData({ name: s.name, class: s.class });
                        setIsModalOpen(true);
                      }}
                      className="p-2.5 text-blue-600 bg-blue-50 rounded-xl hover:bg-blue-100 transition-colors"
                    >
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingId ? 'Editar Aluno' : 'Novo Aluno'}>
        <form
          onSubmit={e => {
            e.preventDefault();
            onSave(formData, editingId || undefined);
            setIsModalOpen(false);
          }}
          className="space-y-5"
        >
          <div className="space-y-1">
            ...
            <input
              ...
              required
              className="w-full p-4 border-2 border-gray-50 rounded-2xl bg-gray-50 outline-none focus:border-blue-600 transition-all font-bold"
              value={formData.name}
              onChange={e => setFormData({ ...formData, name: e.target.value })}
            />
          </div>
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Turma / Período</label>
            <select
              className="w-full p-4 border-2 border-gray-50 rounded-2xl bg-gray-50 font-black outline-none focus:border-blue-600 transition-all"
          ...
          <button
            type="submit"
            className="w-full bg-blue-600 text-white font-black py-4 rounded-2xl shadow-lg shadow-blue-100 uppercase tracking-widest text-sm transition-all hover:bg-blue-700"
          >
            Salvar Aluno
          </button>
        </form>
...
const Transactions: React.FC<{
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-6 rounded-3xl border border-gray-100">
        <h2 className="text-2xl font-black italic uppercase tracking-tighter">
          Movimentação <span className="text-gray-400 not-italic">de Estoque</span>
        </h2>
        <button
          onClick={() => setShowAdd(!showAdd)}
          className={`px-6 py-3 rounded-2xl font-black transition-all flex items-center gap-2 uppercase tracking-widest text-xs ${
            showAdd ? 'bg-gray-100 text-gray-600' : 'bg-gray-900 text-white shadow-xl shadow-gray-200'
          }`}
        >
      ...
      {showAdd && (
        <form
          ...
          className="bg-white p-8 rounded-[2rem] border-2 border-blue-500 grid md:grid-cols-5 gap-5 items-end animate-in slide-in-from-top-4 shadow-2xl"
        >
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Data</label>
            <input
              type="date"
              required
              className="w-full p-3 border-2 border-gray-50 rounded-xl bg-gray-50 font-bold"
              value={formData.date}
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Operação</label>
            <select
              className="w-full p-3 border-2 border-gray-50 rounded-xl bg-gray-50 font-bold"
              value={formData.type}
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Referência</label>
            <input
              type="text"
              placeholder="NF, Memorando..."
              required
              className="w-full p-3 border-2 border-gray-50 rounded-xl bg-gray-50 font-bold"
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Tarefa</label>
            <select
              className="w-full p-3 border-2 border-gray-50 rounded-xl bg-gray-50 font-bold"
          ...
          <div className="flex gap-3">
            <div className="space-y-1 flex-1">
              <label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Qtd</label>
              <input
                type="number"
                required
                className="w-full p-3 border-2 border-gray-50 rounded-xl bg-gray-50 font-bold"
            ...
            <button
              type="submit"
              className="bg-green-600 text-white px-6 py-3 rounded-xl font-black shadow-lg shadow-green-100 transition-all hover:bg-green-700"
            >
              OK
            </button>
          </div>
        ...
        <table className="w-full text-left">
          <thead className="bg-gray-50 border-b uppercase text-[10px] font-black text-gray-400 tracking-widest">
            <tr>
              ...
              <th className="p-5 text-center">Estornar</th>
            </tr>
          </thead>
          <tbody>
            {transactions.map(t => (
              <tr key={t.id} className="hover:bg-gray-50 border-b last:border-0 transition-colors">
                <td className="p-5 text-xs font-bold text-gray-400">{formatDate(t.date)}</td>
                <td className="p-5">
                  <span
                    className={`px-4 py-1.5 rounded-full text-[10px] font-black tracking-widest ${
                      t.type === TransactionType.ENTRY ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                    }`}
                  >
                    {t.type}
                  </span>
                </td>
                <td className="p-5 font-black text-gray-900">{t.partId}</td>
                <td className="p-5 text-right font-mono font-black text-lg">{t.quantity}</td>
                <td className="p-5 text-center">
                  <button
                    onClick={() => deleteTransaction(t.id)}
                    className="text-gray-300 hover:text-red-600 p-2.5 transition-colors bg-gray-50 rounded-xl"
                  >
                    <Trash2 size={16} />
                  </button>
                </td>
