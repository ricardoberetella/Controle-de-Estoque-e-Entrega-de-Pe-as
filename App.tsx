...
const App: React.FC = () => {
  ...
  }, [transactions, withdrawals, parts, students]);
      ...
      <div className="min-h-screen bg-[#F8FAFC] flex flex-col md:flex-row">
        ...
        <main className="flex-1 p-4 md:p-10 pb-24 min-w-0">
          ...
          <Routes>
            ...
            <Route path="/withdrawals" element={<MaterialWithdrawals withdrawals={withdrawals} toggleWithdrawal={toggleWithdrawal} students={students} parts={[...parts].sort((a,b) => sortTaskIds(a.id, b.id))} summary={stockSummary} />} />
            <Route path="/transactions" element={<Transactions transactions={transactions} saveTransaction={handleSaveTransaction} deleteTransaction={handleDeleteTransaction} parts={[...parts].sort((a,b) => sortTaskIds(a.id, b.id))} />} />
      ...
      <div className="bg-white rounded-[2rem] shadow-xl shadow-gray-200/50 border overflow-x-auto min-h-[400px]">
        <table className="w-full text-left border-collapse">
          <thead className="bg-gray-900 text-white">
            <tr><th className="p-5 border-r border-gray-800 sticky left-0 bg-gray-900 z-20 uppercase text-xs font-black tracking-widest">Estudante</th>{sortedParts.map(p => {
...
const Transactions: React.FC<{ transactions: Transaction[], saveTransaction: (t: any, id?: string) => void, deleteTransaction: (id: string) => void, parts: Part[] }> = ({ transactions, saveTransaction, deleteTransaction, parts }) => {
  ...
  useEffect(() => {
    if (sortedParts.length > 0 && !formData.partId) {
      setFormData(prev => ({ ...prev, partId: sortedParts[0].id }));
    }
  }, [sortedParts, formData.partId]);
  ...
  const handleToggleForm = () => {
    if (showAdd) {
      setShowAdd(false);
      setEditingId(null);
      setFormData({ date: new Date().toISOString().split('T')[0], type: TransactionType.ENTRY, description: '', partId: sortedParts[0]?.id || '', quantity: 0 });
    } else {
      setShowAdd(true);
    }
  };
    ...
    <div className="space-y-6">
      ...
      {showAdd && (
        <form onSubmit={(e) => { e.preventDefault(); saveTransaction(formData, editingId || undefined); setShowAdd(false); setEditingId(null); }} className={`bg-white p-8 rounded-[2rem] border-2 ${editingId ? 'border-orange-500' : 'border-blue-500'} grid grid-cols-1 md:grid-cols-6 gap-5 items-end animate-in slide-in-from-top-4 shadow-2xl`}>
          ...
          <div className="space-y-1"><label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Tarefa</label><select className="w-full p-3 border-2 border-gray-50 rounded-xl bg-gray-50 font-bold" value={formData.partId} onChange={e => setFormData({ ...formData, partId: e.target.value })}>{sortedParts.map(p => <option key={p.id} value={p.id}>{p.id} - {p.name}</option>)}</select></div>
