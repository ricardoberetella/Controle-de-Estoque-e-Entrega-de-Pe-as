          ...
          <div className="p-8 border-b border-gray-800 bg-gray-950 flex justify-between items-center">
            <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-col">
                <h1 className="text-white font-bold text-base tracking-tight uppercase leading-none">Mecânico</h1>
                <span className="text-gray-500 font-medium text-[9px] tracking-widest uppercase mt-0.5">Usinagem Convencional</span>
              </div>
            </div>
            <div>
...
const NavItem: React.FC<{ to: string, icon: React.ReactNode, label: string }> = ({ to, icon, label }) => {
  ...
  return (<Link to={to} className={`flex items-center space-x-3 p-3 rounded-xl transition-all ${active ? 'bg-gray-800 text-white shadow-lg' : 'text-gray-400 hover:bg-gray-800 hover:text-white'}`}>{icon}<span className="font-bold text-sm">{label}</span></Link>);
};

const MobileNavItem: React.FC<{ to: string, icon: React.ReactNode }> = ({ to, icon }) => {
  const location = useLocation();
  const active = location.pathname === to;
  return (<Link to={to} className={`p-3 rounded-2xl ${active ? 'bg-gray-100 text-gray-900' : 'text-gray-400'}`}>{icon}</Link>);
};
...
const Dashboard: React.FC<{ summary: StockSummary[], students: Student[] }> = ({ summary, students }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      {/* Cabeçalho Proeminente com Logo SENAI ajustado */}
      <header className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-xl shadow-gray-200/50 border border-gray-100 flex flex-col md:flex-row items-center gap-8 md:gap-14">
        <div className="flex-shrink-0">
          <SenaiLogo sizeClass="text-6xl md:text-8xl" />
        </div>
        
        <div className="text-center md:text-left flex-1 space-y-4">
          <div className="space-y-1">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 leading-tight uppercase tracking-tight">
              Mecânico de Usinagem Convencional
            </h2>
            <p className="text-lg md:text-xl font-medium text-gray-500 uppercase tracking-tight">
              Monitoramento de Estoque e Materiais
            </p>
          </div>
          
          <div className="flex flex-wrap justify-center md:justify-start gap-3 pt-2">
            <span className="inline-flex items-center gap-1.5 px-4 py-1.5 bg-gray-50 text-gray-600 text-xs font-bold uppercase tracking-widest rounded-full border border-gray-200">
            ...
            {summary.filter(s => s.toBuy > 0).slice(0, 5).map(item => (
              <div key={item.partId} className="flex justify-between items-center p-4 bg-gray-50 hover:bg-red-50/50 rounded-2xl border border-gray-100 transition-colors">
                <div className="flex items-center gap-3">
                  <div className="h-10 w-10 flex items-center justify-center bg-white rounded-xl font-black text-gray-600 border border-gray-200">{item.partId}</div>
      ...
      <div className="bg-white rounded-[2rem] shadow-xl shadow-gray-200/50 border overflow-x-auto min-h-[400px]">
        <table className="w-full text-left border-collapse">
          <thead className="bg-gray-900 text-white">
            <tr><th className="p-5 border-r border-gray-800 sticky left-0 bg-gray-900 z-20 uppercase text-xs font-black tracking-widest">Estudante</th>{parts.map(p => (<th key={p.id} className="p-3 text-center border-r border-gray-800 text-[10px] font-black min-w-[65px] uppercase tracking-tighter">{p.id}</th>))}</tr>
          </thead>
          <tbody>
            {filteredStudents.map((student, idx) => (
              <tr key={student.id} className={idx % 2 === 0 ? 'bg-gray-50/50' : 'bg-white'}>
                ...
                {parts.map(part => {
                  ...
                  return (<td key={part.id} className={`p-2 border-r border-gray-100 text-center cursor-pointer transition-colors ${active ? 'bg-green-100/50' : 'hover:bg-blue-50'}`} onClick={() => toggleWithdrawal(student.id, part.id)}>
                    <div className={`mx-auto h-7 w-7 rounded-xl border-2 transition-all flex items-center justify-center ${active ? 'bg-green-500 border-green-600 shadow-md shadow-green-100' : 'border-dashed border-gray-300'}`}>{active && <CheckCircle2 size={16} className="text-white" />}</div>
                  </td>);
                })}
              </tr>
            ))}
            {filteredStudents.length === 0 && (
              <tr><td colSpan={parts.length + 1} className="p-20 text-center font-bold text-gray-300 uppercase tracking-widest">Nenhum aluno nesta turma</td></tr>
            )}
          </tbody>
        </table>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-white p-6 rounded-3xl border border-gray-100">
        ...
        <button onClick={handleOpenAdd} className="bg-gray-900 text-white px-6 py-3 rounded-2xl font-black shadow-lg shadow-gray-100 flex items-center gap-2 transition-transform hover:scale-105 uppercase tracking-widest text-xs"><PlusCircle size={20} /> Nova Peça</button>
      </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingPart ? "Editar Peça" : "Nova Peça"}>
        <form onSubmit={(e) => { e.preventDefault(); onSave(formData, !!editingPart); setIsModalOpen(false); }} className="space-y-5">
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1"><label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Identificação</label><input type="text" placeholder="ex: T1" required disabled={!!editingPart} className="w-full p-4 border-2 border-gray-50 rounded-2xl bg-gray-50 outline-none focus:border-gray-500 transition-all font-bold" value={formData.id} onChange={e => setFormData({ ...formData, id: e.target.value.toUpperCase() })} /></div>
            <div className="space-y-1"><label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Meta Turma</label><input type="number" placeholder="0" required className="w-full p-4 border-2 border-gray-50 rounded-2xl bg-gray-50 outline-none focus:border-gray-500 transition-all font-bold" value={formData.targetQuantity} onChange={e => setFormData({ ...formData, targetQuantity: parseInt(e.target.value) || 0 })} /></div>
          </div>
          <div className="space-y-1"><label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Código Referência</label><input type="text" placeholder="Código SAP/Referência" className="w-full p-4 border-2 border-gray-50 rounded-2xl bg-gray-50 outline-none focus:border-gray-500 transition-all font-bold" value={formData.code} onChange={e => setFormData({ ...formData, code: e.target.value })} /></div>
          <div className="space-y-1"><label className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Nome da Peça</label><input type="text" placeholder="Nome descritivo" required className="w-full p-4 border-2 border-gray-50 rounded-2xl bg-gray-50 outline-none focus:border-gray-500 transition-all font-bold" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} /></div>
          <button type="submit" className="w-full bg-gray-900 text-white font-black py-4 rounded-2xl shadow-lg shadow-gray-200 uppercase tracking-widest text-sm transition-all hover:bg-black">Salvar Alterações</button>
        </form>
      </Modal>
    </div>
  );
  ...
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {summary.filter(s => s.toBuy > 0).map(item => (
        <div key={item.partId} className="bg-white p-8 rounded-[2rem] shadow-sm border-t-8 border-gray-900 flex justify-between items-center group transition-all hover:shadow-xl hover:-translate-y-1">
      ...
      <div className="bg-white rounded-[2rem] border overflow-hidden shadow-sm">
        <table className="w-full text-left">
          <tbody className="divide-y divide-gray-100">
            ...
            {filtered.length === 0 && (
              <tr><td colSpan={2} className="p-10 text-center text-gray-400 font-bold uppercase tracking-widest">Nenhum aluno cadastrado nesta turma</td></tr>
            )}
          </tbody>
        </table>
      </div>
...
const Transactions: React.FC<{ transactions: Transaction[], addTransaction: (t: any) => void, deleteTransaction: (id: string) => void, parts: Part[] }> = ({ transactions, addTransaction, deleteTransaction, parts }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {showAdd && (
        <form onSubmit={(e) => { e.preventDefault(); addTransaction(formData); setShowAdd(false); }} className="bg-white p-8 rounded-[2rem] border-2 border-gray-900 grid md:grid-cols-5 gap-5 items-end animate-in slide-in-from-top-4 shadow-2xl">
